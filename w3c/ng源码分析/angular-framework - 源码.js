! function(window, document, undefined) {
	...
	//继承，创建一个新对象，将其prototype设置为parent，与extra合并后返回
	...
	//【1.3】document的dom加载完毕，ready事件执行
	//element为document
	function angularInit(element, bootstrap) {
		...
		//appElement为设置ng-app的DOM对象;将document加入elements
		//根据names判断启动DOM（appElement），ID-类-属性
		...
		forEach(names, function(name) {
			...
			if (!appElement) {//若为类设置的启动，则module从value中获取
		...
		}), appElement && bootstrap(appElement, module ? [module] : [])//若找到启动项，则执行bootstrap，开始启动。【1.4】
	}
	//【1.4】启动函数
	//element为启动DOM,modules为模块名数组["myAPP"]，未设置时为[]
	function bootstrap(element, modules) {
		//启动方法，手工启动时也执行这里
		var doBootstrap = function() {
			//将启动DOM封装为JQ对象，若有injector方法则报错
			if (element = jqLite(element), element.injector()) {
				var tag = element[0] === document ? "document" : startingTag(element);
				throw ngMinErr("btstrpd", "App Already Bootstrapped with this Element '{0}'", tag)
			}
			//将["$provide",fun]放入modules首位
			modules = modules || [], modules.unshift(["$provide",
				function($provide) {
					$provide.value("$rootElement", element)
				}
			]), modules.unshift("ng");//将ng放入首位
			//目前modules是[["$provide",fun],"ng",modules]
			//创建注射器
			...
			*/
			//由于注入，会依次实例化$rootScope, $rootElement, $compile, $injector
			//$injector在946行providerInjector = providerCache.$injector 创建
		...
		//window.name不为NG_DEFER_BOOTSTRAP，调用doBootstrap
		//若为NG_DEFER_BOOTSTRAP，将window.name置空，为angular添加angular.resumeBootstrap方法，用于重新启动
		//resumeBootstrap方法方法，接受一个参数模块数组，执行时，将数组压入modules作为参数，执行doBootstrap
	...
	//[1.1]绑定Jquery对象
	function bindJQuery() {
		//若系统已有jQuery,赋值给jqLite，并scope等方法添加到jQuery实例，重写remove，empty，html方法，为了触发$destroy等事件
		//若不包含，将jqLite和angular.element设置为JQLite
	...
	function setupModuleLoader(window) {
		...
		var $injectorMinErr = minErr("$injector"),
			ngMinErr = minErr("ng");
		//为angular对象添加module方法
		return ensure(ensure(window, "angular", Object), "module", function() {
			var modules = {};//记录所有模块
			//module方法内容：angular.module=function(name, requires, configFn) {
			return function(name, requires, configFn) {
				//name为hasOwnProperty则报错
				var assertNotHasOwnProperty = function(name, context) {
					if ("hasOwnProperty" === name) throw ngMinErr("badname", "hasOwnProperty is not a valid {0} name", context)
				};
				//modules是430行的数组，将name保存起来，其值为一个函数，返回moduleInstance
				return assertNotHasOwnProperty(name, "module"), requires && modules.hasOwnProperty(name) && (modules[name] = null), ensure(modules, name, function() {
					//在invokeQueue添加invokeQueue.insertMethod(provider, method, arguments);
					//或invokeQueue.push(provider, method, arguments);
					//返回一个函数，该函数返回moduleInstance模块实例
					...
					//定义invokeQueue队列，runBlocks依赖模块加载队列，config模块
					var invokeQueue = [],
						runBlocks = [],
						//将config设置为一个FUN，逻辑为：invokeQueue.push(["$injector", "invoke",arguments]);
						config = invokeLater("$injector", "invoke"),
						//定义模块实例，初始化相关属性
						moduleInstance = {
							_invokeQueue: invokeQueue,//invokeQueue时数组包含了push内的元素
							_runBlocks: runBlocks,
							requires: requires,
							name: name,
							//将provider设置为一个FUN，逻辑为：invokeQueue.push(["$provide", "provider",arguments]);
							provider: invokeLater("$provide", "provider"),
							//将factory设置为一个FUN，逻辑为：invokeQueue.push(["$provide", "factory",arguments]);
							factory: invokeLater("$provide", "factory"),
							//将service设置为一个FUN，逻辑为：invokeQueue.push("$provide", "service",arguments]);
							service: invokeLater("$provide", "service"),
							//将value设置为一个FUN，逻辑为：invokeQueue.push("$provide", "value",arguments]);
							value: invokeLater("$provide", "value"),
							//将constant设置为一个FUN，逻辑为：invokeQueue.unshift("$provide", "constant",arguments);
							constant: invokeLater("$provide", "constant", "unshift"),
							//将animation设置为一个FUN，逻辑为：invokeQueue.push("$animateProvider", "register",arguments);
							animation: invokeLater("$animateProvider", "register"),
							//将filter设置为一个FUN，逻辑为：invokeQueue.push("$filterProvider", "register",arguments);
							filter: invokeLater("$filterProvider", "register"),
							//将controller设置为一个FUN，逻辑为：invokeQueue.push("$controllerProvider", "register");
							controller: invokeLater("$controllerProvider", "register"),
							//将directive设置为一个FUN，逻辑为：invokeQueue.push("$compileProvider", "directive");
							directive: invokeLater("$compileProvider", "directive"),
							config: config,
							//run方法，将方法加入runBlocks块，等待实例化时执行
							run: function(block) {
								return runBlocks.push(block), this
							}
						};
						//若configFn不为空，将执行invokeQueue.push(["$injector", "invoke",configFn]);
					return configFn && config(configFn), moduleInstance//返回moduleInstance，可以链式调用
				})
			}
		})
	}
	//【1.2】使用extend发布下面的方法，链接到angular对象上
	//设置$compileProvider
	function publishExternalAPI(angular) {
		extend(angular, {
			...
			$$minErr: minErr,
			$$csp: csp
		}), angularModule = setupModuleLoader(window);//添加module方法
		try {
			angularModule("ngLocale")//获取ngLocale模块
		} catch (e) {
			angularModule("ngLocale", []).provider("$locale", $LocaleProvider)//创建ngLocale和ng模块
		}
		//ng模块，设置$compile服务 angular.module("ng", ["ngLocale"], ["$provide"...);
	...
	//【1.5】为创建注入器，内部会创建两个注入器
	//providerInjector用于
	//instanceInjector用于
	//函数的返回值为instanceInjector
	function createInjector(modulesToLoad) {
		//返回一个函数，逻辑为：若为对象，遍历对象，执行delegate 将value key作为参数
		//若是函数，执行传入的delegate方法，将key value作为参数
		function supportObject(delegate) {
			return function(key, value) {
				return isObject(key) ? (forEach(key, reverseParams(delegate)), void 0) : delegate(key, value)
			}
		}
		//若name不为hasownproperty,provider_为函数或数组
		//providerInjector实例化provider_   provider_必须有$get属性。
		...
		//调用provider，传入name和{$get: factoryFn}
		function factory(name, factoryFn) {
			return provider(name, {
				$get: factoryFn
			})
		}
		//调用factory，传入name,["$injector",fn(使用注入器实例化fn)]
		//再调用provider，传入name和{$get: ["$injector",fn(使用注入器实例化fn)]}
		...
		//调用factory，传入传入name,["$injector",fn(使用注入器实例化fn，fn返回值为val)]
		//再调用provider，传入name和{$get: ["$injector",fn(使用注入器实例化fn，fn返回值为val)]}
		function value(name, val) {
			return factory(name, valueFn(val))//valueFn逻辑为，返回一个函数，这个函数返回val
		}
		//name不为ownxx，将name放在模块的providerCache缓存中和instanceCache缓存
		function constant(name, value) {
			assertNotHasOwnProperty(name, "constant"), providerCache[name] = value, instanceCache[name] = value
		}
		//装饰器，内部获取了实例，并执行了$get，随后可以执行decorFn
		function decorator(serviceName, decorFn) {
			//providerInjector为模块创建的注入器，通过xxxProvider获取服务实例，orig$get为provider的$get方法
			//origProvider的$get设置为函数，逻辑为：先执行传入serviceProvider实例的$get，再执行decorFn函数
		...
		//功能：加载模块
		//返回runBlocks
		//首次调用时，[ng，[$provider,fun],模块名称]
		function loadModules(modulesToLoad) {
			var moduleFn, invokeQueue, i, ii, runBlocks = [];
			//遍历，若已加载数组loadedModules不包括，则加载：压入loadedModules,其值设置为!0。
			//若为字符串，获取模块，遍历获取runBlocks，依次调用，invokeQueue时数组，记录了模块的加载时需要执行的方法。
		...
		function createInternalInjector(cache, factory) {
			//getService查找服务，cache包括则直接返回，不包括则压入并实例化（factory为创建注入器时注入），
			...
			//先调用annotate分析方法，获取参数。根据获取的参数注入：locals包含此参数则注入，包含则使用getService获取服务实例。
			//若self不为空， fn.apply(self, args)；若为空，使用注入的参数
	...
	function $CompileProvider($provide) {
		...
		//directive方法是一个函数，加载指定的指令，返回指令数组
		//name传入为{name:xxxDirective,...}
		this.directive = function registerDirective(name, directiveFactory) {
			return assertNotHasOwnProperty(name, "directive"),
			 //若为字符串，参数directiveFactory不为空，
			 //若指令中也包含该指令，获取之，将directiveFactory压入，结束
			 //若不包含，则hasDirectives[name]为空数组，注册nameDirective指令
			 
			 isString(name) ? (assertArg(directiveFactory, "directiveFactory"), 
			 					hasDirectives.hasOwnProperty(name) || (hasDirectives[name] = [], 
			 						//内部逻辑为：遍历数组，回调factory，若为函数，将其指定为compile函数，
				...
				function($injector, $exceptionHandler) {
					...
					return forEach(hasDirectives[name], function(directiveFactory, index) {
						try {
							...
							//不为函数，若compile不存在，link存在，则compile设置为link
							: !directive.compile && directive.link && (directive.compile = valueFn(directive.link)), 
							directive.priority = directive.priority || 0,//初始化
							directive.index = index, 
							directive.name = directive.name || name, 
							directive.require = directive.require || directive.controller && directive.name, 
							directive.restrict = directive.restrict || "A",
							directives.push(directive)//将指令压入
						} catch (e) {
							$exceptionHandler(e)
						}
					}), directives
			...
			//如果是对象，遍历调用registerDirective方法，参数为翻转后的,
			: forEach(name, reverseParams(registerDirective)), this
		}, 
		...
		this.$get = ["$injector", "$interpolate", "$exceptionHandler", "$http", "$templateCache", "$parse", "$controller", "$rootScope", "$document", "$sce", "$animate",
			function($injector, $interpolate, $exceptionHandler, $http, $templateCache, $parse, $controller, $rootScope, $document, $sce, $animate) {
				...
				//compile方法：
				//compileNodes节点,第一次从379行调用compile(element)(scope)
				function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
					//$compileNodesdom对象则封装为jquery对象，
					$compileNodes instanceof jqLite || ($compileNodes = jqLite($compileNodes)), 
					//遍历jquery对象，若节点为文本节点，且文本值不为空字符串，使用<span></span>包装文本
				...
				function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective, previousCompileContext) {
					...
					for (var nodeLinkFn, childLinkFn, directives, attrs, linkFnFound, linkFns = [], i = 0; i < nodeList.length; i++) 
						...
						childLinkFn = nodeLinkFn && nodeLinkFn.terminal || !nodeList[i].childNodes || !nodeList[i].childNodes.length 
							? null : compileNodes(nodeList[i].childNodes, nodeLinkFn ? nodeLinkFn.transclude : transcludeFn), //遍历子节点
						...
						case 1://元素节点，尝试添加指令 1437行；
							...
							for (var attr, name, nName, ngAttrName, value, nAttrs = node.attributes, j = 0, jj = nAttrs && nAttrs.length; jj > j; j++) {
								...
								if (attr = nAttrs[j], !msie || msie >= 8 || attr.specified) {
									...
									getBooleanAttrName(node, nName) && (attrs[nName] = !0), //输入域相关的布尔属性
				...
				//node为input 为当前所有的directives  value为name  name为ngModel
	...
	//【1.7】这部份代码其实可以独立出来，做成自己的计算器程序也是可以的,
	//因为它的核心就是解析字符串,而且默认支持四则运算,运算符号的优先级处理,
	//只是额外的增加了对变量的支持以及过滤器的支持
	function $ParseProvider() {
		...
		this.$get = ["$filter", "$sniffer", "$log",
			function($filter, $sniffer, $log) {//注入了三个服务
	...
	//返回一个对象{defer: defer,reject: reject,when: when,all: all}
		//defer为一个函数，返回promise对象{resolve: function, reject: function, notify: function, promise: Object}
		//reject
	function qFactory(nextTick, exceptionHandler) {
		...
		//defer为一个函数，
		//$q.defer()返回一个deffered实例{resolve: function, reject: function, notify: function, promise: Object}
		//var deferred=$q.defer(); var promise=deferred.promise;promise.then();
		//resolve:承诺  reject:拒绝  notify:通知
		var defer = function() {
			...
			return deferred = {
				...
				resolve: function(val) {
					if (pending) {
						...
						pending = undefined,
						value = ref(val), //value.then=fun(callback){result.resolve(callback(value))压入异步队列，返回promise}；
						callbacks.length && nextTick(function() {//若callbacks不为空，将加入队列
							for (var callback, i = 0, ii = callbacks.length; ii > i; i++) 
								callback = callbacks[i], value.then(callback[0], callback[1], callback[2])
						})
					}
				},
				//拒绝promise，将reject构造的拒绝对象传给resolve
				reject: function(reason) {
					deferred.resolve(reject(reason))//reject(reason)2462行，
				},
				//在promise执行过程中提供状态更新，在reject或resolve之前可能被多次调用
				...
				promise: {
					then: function(callback, errback, progressback) {
						var result = defer(),
							...
							//将[success,error,progress]压入pending
						return pending ? pending.push([wrappedCallback, wrappedErrback, wrappedProgressback]) 
		...
		//若value不为空，且value.then为函数，返回value
		//否则为value设置then该方法，该方法创建一个延迟对象，将压入延迟队列
		ref = function(value) {
	...
	function $RootScopeProvider() {
		...
		this.digestTtl = function(value) {
			...
			function($injector, $exceptionHandler, $parse, $browser) {
				...
				//$$listeners, 代表scope变量当前所有的监听事件$on,是一个数组
				//$$listenerCount, 暂无
				//$$isolateBindings, 暂无
				function Scope() {
				...
				//开始一个阶段,
				//若$rootScope.$$phase不为空，表示正在运行中，抛出异常
				//将$rootScope.$$phase设置为传递的参数
				function beginPhase(phase) {
					if ($rootScope.$$phase) throw $rootScopeMinErr("inprog", "{0} already in progress", $rootScope.$$phase);
					$rootScope.$$phase = phase
				}
				//结束一个阶段，将$rootScope.$$phase设置为空
				function clearPhase() {
					$rootScope.$$phase = null
				}
				//调用$parse实例来分析监控参数,然后返回一个函数
				...
				Scope.prototype = {
					constructor: Scope,
					//创建子级作用域是通过$new方法,isolate标识来创建独立作用域
					//如果不是独立的作用域,则会生成一个内部的构造函数,把此构造函数的prototype指向当前scope实例
					$new: function(isolate) {
						var Child, child;
						return isolate 
						? (child = new Scope, //独立作用域，
							child.$root = this.$root, //设置为$rootScope
							child.$$asyncQueue = this.$$asyncQueue, //
							child.$$postDigestQueue = this.$$postDigestQueue) 
						: (Child = function() {}, 
							Child.prototype = this, //创建一个实例，其prototype指向Scope.prototype,即Child继承调用$new的当前类实例
							child = new Child, 
							child.$id = nextUid()), //新创建id
							child["this"] = child, 
							child.$$listeners = {}, 
							child.$parent = this, 
							child.$$watchers = child.$$nextSibling = child.$$childHead = child.$$childTail = null, //初始化为null
							child.$$prevSibling = this.$$childTail, 
							//若父scope的childHead存在，则将child放在队尾
							this.$$childHead ? (this.$$childTail.$$nextSibling = child, this.$$childTail = child) 
								:this.$$childHead = this.$$childTail = child,//不存在则头尾均设置为当前节点
						child//返回child
					},
					//第一个是监控参数,可以是字符串或者函数,第二个是监听函数,第三个是代表是否深度监听
					$watch: function(watchExp, listener, objectEquality) {
						var scope = this,
							get = compileToFn(watchExp, "watch"),//分析监控参数，返回一个函数
							array = scope.$$watchers,//
							watcher = {
								fn: listener,//监听函数,当监控表达式新旧不相等时会执行此函数
								last: initWatchVal,//初始化为空对象，保存最后一次发生变化的监控表达式的值
								get: get,//一个函数，返回监控参数，保存一个监控表达式对应的函数,目的是用来获取表达式的值然后用来进行新旧对比的
								exp: watchExp,//监控参数表达式
								eq: !! objectEquality//是否深度监控（对象的属性）
							};
						if (!isFunction(listener)) {//listener不为函数
							var listenFn = compileToFn(listener || noop, "listener");
							watcher.fn = function(newVal, oldVal, scope) {
								listenFn(scope)
							}
						}
						if ("string" == typeof watchExp && get.constant) {//字符串
							var originalFn = watcher.fn;//传入的监听器
							watcher.fn = function(newVal, oldVal, scope) {//设置监听函数，逻辑为调用监听器，传入newVal, oldVal, scope
								originalFn.call(this, newVal, oldVal, scope), arrayRemove(array, watcher)//将watcher从scope.$$watchers中移除
							}
						}
						//如array为空，设置为空数组[],最后将将watcher加入scope.$$watchers,(回调时会移出)
						//返回一个函数，逻辑为将watcher从array中移除，这个就是用来删除监听用的。
					...
					//digest方法是dirty check的核心,
					//主要思路是：1.先执行$$asyncQueue队列中的表达式,
					//2.开启一个loop来的执行所有的watch里的监听函数,前提是前后两次的值是否不相等,假如ttl超过系统默认值，则dirth check结束,
					//3.最后执行$$postDigestQueue队列里的表达式.
					$digest: function() {
						var watch, value, last, watchers, length, 
							dirty, 
							next, 
							current, //当前scope
							...
							ttl = TTL,
							target = this,//默认为本scope
							watchLog = [];
						//开始$digest阶段
						beginPhase("$digest");
						do {
							//遍历asyncQueue 异步队列 并执行
							...
							do {
								if (watchers = current.$$watchers)//若当前当前scope的$$watchers不为空
									for (length = watchers.length; length--;)
									 try {	
										watch = watchers[length], //最后一个
										watch && (value = watch.get(current)) !== (last = watch.last) ////前后两次的值不相等
										//深度检查标志为true：比较本地和上次，相等则结束，继续循环；为false，当前值和上次值都为数字且不为NAN，则结束，继续循环
										&& !(watch.eq ? equals(value, last) : "number" == typeof value && "number" == typeof last && isNaN(value) && isNaN(last)) 
										&& (dirty = !0, 
											watch.last = watch.eq ? copy(value) : value, //若深度，则拷贝一份 赋值给value，否则直接赋值
											watch.fn(value, last === initWatchVal ? value : last, current), //执行watch的监听函数
											5 > ttl && (logIdx = 4 - ttl, watchLog[logIdx] || (watchLog[logIdx] = []),
								...
								//将next赋值为当前的子节点头，若存在，则继续循环，不执行下面的for；
															 //若不存在子，当前和本scope不相同则继续循环，不执行下面的for；
															 //若不存在子，当前和本scope相同，当前的兄弟不存在，则循环，不存在兄弟，不for继续循环
								//不断的查找当前作用域的子级,没有子级，则开始查找兄弟节点,最后查找它的父级节点,是一个深度遍历查找.只要next有值,则内loop则一直执行
								if (!(next = current.$$childHead || current !== target && current.$$nextSibling))
									//当前scope和本scope不相同，将next赋值为当前的兄弟节点
					...
					$destroy: function() {
						//rootScope不等于当前scope，且未destroy过，则销毁
					...
					//evalAsync函数的作用就是延迟执行表达式,并且执行完不管是否异常,触发dirty check.【$rootScope.$digest()】
					//加入$$asyncQueue，这是异步队列，保存着所有需要延迟执行的表达式,此处的表达式可以是字符串或者函数,因为这个表达式最终会调用$eval方法
					//调用了$browser服务的defer方法,从ng->browser.js源码里可以看到,其实这里就是调用setTimeout来实现的.
					...
					//不会主动触发digest方法,只是往postDigestQueue队列中增加执行表达式，
					//它会在digest体内最后执行，相当于在触发dirty check之后，可以执行别的一些逻辑.
					...
					$apply: function(expr) {
						try {
							return beginPhase("$apply"), this.$eval(expr)//开始$apply阶段，执行expr
						} catch (e) {
							$exceptionHandler(e)
						} finally {
							clearPhase();//清除阶段
							try {
								$rootScope.$digest()//开始脏检查，来使ng中的M或者VM改变.
							} catch (e) {
					...
					$on: function(name, listener) {
						var namedListeners = this.$$listeners[name];//获取保存到监听事件，
						//若存在，则返回，不存在，则压入
					...
					//用来触发$on定义的事件,原理就是loop$$listeners属性,检查是否有值,有的话，则执行,然后依次往上检查父级,这个方法有点类似冒泡执行事件.
					$emit: function(name) {
				...
				//系统默认会创建根作用域，并作为$rootScopeprovider实例返回.
	...
	var msie, jqLite, jQuery, angularModule, nodeName_, slice = [].slice,
		...
		ngMinErr = minErr("ng"),
		angular = (window.angular, window.angular || (window.angular = {})),//设置angular初始值
		uid = ["0", "0", "0"];
	...
	//[1.0]开始执行，启动》》绑定jquery》》发布函数，添加module方法并创建ngLocale和ng模块
	bindJQuery(), publishExternalAPI(angular), jqLite(document).ready(function() {
		angularInit(document, bootstrap)//Dom加载完成执行angularInit，bootstrap为启动函数，实例化ngLocale和ng
	})
