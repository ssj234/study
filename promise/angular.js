...
function qFactory(nextTick, exceptionHandler) {
   ...
   */
  var defer = function(_name) {
    console.log('create defer'+_name);
    var pending = [],//保存then中指定的回调函数
        value, //这个一个包装后的类promise对象
        deferred;//延迟对象

    deferred = {

      resolve: function(val) {
        if (pending) {//首次resolve，[]为true
          var callbacks = pending;//保存pending，放在callbacks中，
          pending = undefined;//设置pending为空，保证下次不会进入
          value = ref(val);//若val为promise，value为该promise对象
                          //若val不为promise或为空，value为一个对象，有then方法，
                                //then会创建defer，nextTick执行传入的callback，参数为val

          //若有回调函数,表明先then后resolve，
          //若无回调函数,表明then未设置回调或先resolve后then
          if (callbacks.length) {
            //nextTick(function() {//加入队列，val执行完成后回调
              var callback;
              for (var i = 0, ii = callbacks.length; i < ii; i++) {
                callback = callbacks[i];//遍历回调函数，
                //value是包装后的类promise
      ...
      reject: function(reason) {
        //类似于resolve，区别碍于value.then(callback[1])
          ...
          if (pending.length) {
            nextTick(function() {
              ...
              for (var i = 0, ii = callbacks.length; i < ii; i++) {
                ...
                callback[2](progress);//调用progress回调
              }
            });
          }
        }
      ...
      promise: {
        ...
        then: function(callback, errback, progressback) {
          //创建一个defer,then_defer
          var result = defer('_then');

          /*******************==三个内部函数==************************/
          //目的在于，创建回调执行后，调用then_defer的resolve/reject/notify方法
          //以便于then的链式调用，then()方法返回了then_defer
          ...
          //若resolve之前then
          if (pending) {
            //将三个回调函数压入即可，函数会在resolve中处理
            pending.push([wrappedCallback, wrappedErrback, wrappedProgressback]);
          } else {//resolve之后再then方法
            //先resolve时会设置value，这是一个包装后的promise，
                //若先resolve时为非promise对象，会返回一个对象{then:xx}
                //then会创建ref_defer，设置下个tick执行ref_defer.resolve(wrappedCallback(val));

                //若先resolve时为promise对象，value为这个promise
                //then的话相当于为传入的promise对象添加回调
            value.then(wrappedCallback, wrappedErrback, wrappedProgressback);
          }
          //返回then_defer的promise
  ...
  //返回了一个对象，有then方法，
  //promise的then方法可以添加回调
  //其他情况的then方法添加成功回调
  var ref = function(value) {
    //若value是一个promise，返回promise
    if (value && isFunction(value.then)) return value;
    //否则返回一个对象，创建一个defer，返回其promise
    return {
      then: function(callback) {
        var result = defer('_ref');
        nextTick(function() {
